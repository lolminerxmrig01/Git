...
function vray_tmp_config_file_check_and_use() {
    if [[ -s ${config_path} ]]; then
        mv -f ${vray_conf_dir}/config_tmp.json ${config_path}
    else
...
function add_new_user() {
    ...
    if grep -q "vmess" ${config_path} || grep -q "vless" ${config_path}; then
        ...
        cat ${config_path} | jq 'setpath(["inbounds",0,"settings","clients",'${users_count}',"id"];"'${UUID}'")' >${vray_conf_dir}/config_tmp.json
    ...
    cat ${config_path} | jq 'setpath(["inbounds",0,"settings","clients",'${users_count}',"email"];"'${new_user_num}@${new_user_name}'")' >${vray_conf_dir}/config_tmp.json
    vray_tmp_config_file_check_and_use
...
function delete_user() {
    ...
    if [[ ${current_users_count} == "1" ]]; then
        print_error "You can't delete the last user. V2ray Must have at least ONE user."
    ...
    cat ${config_path} | jq 'del(.inbounds[0].settings.clients['${user_number}'])'>${vray_conf_dir}/config_tmp.json
    vray_tmp_config_file_check_and_use
