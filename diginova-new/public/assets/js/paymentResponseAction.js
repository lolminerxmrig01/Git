/*[PATH @digikala/supernova-digikala-desktop/assets/local/js/controllers/paymentController/paymentResponseAction.js]*/
var paymentResponseIndexAction = {

    init: function () {
        this.initEnableSubmitButton();
        this.initCollapseButtons();
        this.initRadioButtonsAction();
        this.sendAdroRetargetingData();
        this.sendAffiliationValidation();
        this.showMorePaymethod();
        this.initShippingTimeTable();
        this.initCartPage();
        this.initSeparateProducts();
        this.initAsideRecommendation();
        this.initCarousel();
        this.initPackageInformSection();

        if(isModuleActive('data_layer')) {
            this.initGATankYouImpression();
        }
    ...
    initAsideRecommendation: function () {
        var pageNumber = 0;
        var products = $('.js-thank-recommendation-product');

        if(products.length < 1)
            return;

        var pages = Math.floor(products.length / 4);

        function updatePage(pageNumber) {
            products.hide();
            $(products[pageNumber*4]).show();
            $(products[pageNumber*4 + 1]).show();
            $(products[pageNumber*4 + 2]).show();
            $(products[pageNumber*4 + 3]).show();
        }

        updatePage(pageNumber);

        $('.js-thank-recommendation-prev').on('click', function () {
            if(pageNumber === 0) {
                pageNumber = pages - 1;
            } else {
                pageNumber --;
        ...
        $('.js-thank-recommendation-next').on('click', function () {
    ...
    initCollapseButtons: function () {
        if (isModuleActive('dk_wallet')) {
            ...
            orderSummaryHeader.after().on('click', function () {
            ...
            orderSummaryHeader.after().click();
    ...
    sendAdroRetargetingData: function () {
        // if (typeof emarsys_order_data === 'undefined') {
        //     return;
        // }
        // var orderDataTemp = emarsys_order_data || [];
        // if (!orderDataTemp || orderDataTemp.length === 0) return;
        // try {
        //     var orderProductData = orderDataTemp.items.map(function (x) {
        //         return x['id'];
        //     });
        //     if (window.adroRCActivation) StickyRetargeting.EventListener(StickyRetargeting.Buy_Products.bind(orderProductData));
        // } catch (e) {
        //     // Adro Retargeting events will not be sent
        // }
    },

    sendAffiliationValidation: function () {
        if ($('.c-checkout-alert__icon').hasClass('success') && typeof SnTracker.utils.getCookieById('affiliate_token', 'string') !== 'undefined') {
            var affiliateId = decodeURIComponent(SnTracker.utils.getCookieById('affiliate_token', 'string'));
            var uvalidAffiliateCookie = SnTracker.utils.getCookieById('uvalid', 'string');
            affiliateId = affiliateId.split(':')[0];
            if (!affiliateId || !uvalidAffiliateCookie) {
                return;
            }
            var validationData = {
                SyncId: "DKC-" + $('.js-checkout_dkc_sn').data('id'),
                AffiliateId: affiliateId,
            };
            $.ajax({
                type: 'POST',
                url: 'https://afftrack.adro.co/Invoice/TrackOrder',
                data: validationData,
                async: true,
                ...
                console.error('Problem in Affiliation Validation');
            });
        } else {
    ...
    initPackageInformSection: function() {
        ...
        $(document).on('click', '.js-inform-package-section', function() {
            var radios = $('.js-checkout-pack input[type="radio"]');
            var keys = {};
            ...
            for(var i=0; i < keysList.length; i++) {
                if(!keys[keysList[i]]) {
                    $([document.documentElement, document.body]).animate({
                        scrollTop: $('input[name="' + keysList[i] + '"]').closest('.js-checkout-pack').offset().top
                    }, 100);
                    break;
                }
            }
    ...
    initEnableSubmitButton: function () {
        var button = $('.js-to-gateway'),
            additionalButton = $('.js-checkout-additional-option'),
            informPackageSection = $('.js-inform-package-section'),
            uncheckedPackageCount = $('.js-unchecked-package-count'),
            radios = $('.js-checkout-pack input[type="radio"]'),
            keys = {};
        ...
        var groups = Object.keys(keys).length,
            activeRadioCount = radios.filter(':visible').filter(':checked').length;
        ...
        additionalButton.on('change', function (e) {
            ...
            if ($(this).is(':checked')) {
                ...
                var $checkoutPack = $(this).closest('.c-checkout-pack'),
                    $mainOption = $checkoutPack.find('input[type="radio"]').not($(this));
        ...
        Services.ajaxGETRequestJSON(
            ...
            function (res) {
                ...
                if (!response.hasOwnProperty('amount') && !response.hasOwnProperty('activationUrl')) {
                    ...
                    thiz.updateWallet(window.payablePrice);
    ...
    initGATankYouImpression: function () {
        var result = $('.js-thank-you-payment-status').data('status');
        if(!window.dataLayerData || !window.dataLayerData.hasOwnProperty('isNewUser')) return false;
        ...
        window.dataLayer.push({
            ...
            ecommerce: {
                purchase: {
                    actionField: {
                        IsNewCustomer: window.dataLayerData.isNewUser
                    }
                }
            }
        })
...
$(function () {
    paymentResponseIndexAction.init();
});
