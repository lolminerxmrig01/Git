...
function Tags(selector, position, validation) {
    ...
    this.phrase = "";
    this.htmlSelect = $(this.selector)
        .closest(".js-textarea-tags")
        .siblings(".js-textarea-tags-select");
    this.tagsArray = this.htmlSelect.val() || [];
    if (validation) {
        this.invalidTags = this.htmlSelect
            .find('option[data-valid="false"]')
            .map(function() {
                return $(this).val();
            });
    }
...
Tags.prototype.eventListeners = function() {
    ...
    if (this.position === "after") {
        $removeContext = $(this.selector)
            .closest(".js-textarea-tags")
            .siblings(".js-textarea-tags-container");
    } else {
...
Tags.prototype.createTag = function() {
    ...
    if (this.phrase.length < 1) {
        ...
        $(this.phrase).each(function(e) {
            if (
                !$that.tagsArray.includes($($that.phrase)[e].trim()) &&
                $($that.phrase)[e].trim().length > 1
            ) {
    ...
    for (let i = initCount; i < this.tagsArray.length; i++) {
        if (this.position === "after") {
            $(`
                <span class="c-ui-tag__label ${
                    $.inArray(this.tagsArray[i], this.invalidTags) > -1
                        ? "has-error"
                        : ""
                }" data-value="${this.tagsArray[i]}">
                    ${this.tagsArray[i]}
    ...
    /* reset input value */
    $(this.selector).val("");
    this.removeTriggers = $(this.selector)
        .siblings()
        .find(".js-tag-remove");
    $(this.selector)
        .closest(".js-textarea-tags")
...
Tags.prototype.initTextareaTagsUpdate = function() {
    if (this.validation) {
        ...
        $(this.selector)
            .closest(".js-textarea-tags")
            .siblings(".js-textarea-tags-select").html(`
            ${this.tagsArray
                .map(
                    tag =>
                        `<option value="${tag}" selected="selected" ${
                            tag.length < $validation.min ||
                            tag.length > $validation.max
                                ? 'data-valid="false"'
                                : ""
                        }>${tag}</option>`
                )
        ...
        $(`
            ${this.tagsArray
                .map(
                    tag =>
                        `<span class="c-ui-tag__label ${
                            $.inArray(tag, this.invalidTags) > -1
                                ? "has-error"
                                : ""
                        }" data-value="${tag}">
                ${tag}
...
(function($) {
    ...
    CircleProgress.prototype = {
        ...
        getThickness: function() {
            return $.isNumeric(this.thickness)
                ? this.thickness
                : this.size / 14;
        },
    ...
    $.fn.circleProgress = function(configOrCommand, commandArgument) {
        ...
        return this.each(function() {
            var el = $(this),
                ...
                config = $.isPlainObject(configOrCommand)
                    ? configOrCommand
                    : {};
            ...
            if (instance) {
                ...
                if (typeof initialConfig.animation == "string")
                    initialConfig.animation = JSON.parse(
...
let IndexAction = {
    ...
    initForceMarketplaceSellerChange: function() {
        ...
        $("#marketplaceSellersList").change(function() {
            window.location.href =
                "/content/create/product/for/marketplace/seller/" +
                $(this).val() +
                "/";
        });
    },
        ...
        $(".js-select-origin").each(function() {
            ...
            $this
                ...
                .on("select2:select", function() {
                    let $sortedOptions = $("li.select2-results__option").sort(
                        function(a, b) {
                            return (
                ...
                .on("change", function() {
                    if (isMultiSelect && inProductStep) {
                        ...
                        $selections.each(function() {
                            $selectionsContainerWidth -= $(this).outerWidth(
                                true
                            );
    ...
    initTextareaTags: function() {
        ...
        new window.Tags(".js-textarea-tags-pros", "after", {
            ...
            max: 50,
            message:
                "متن وارد شده در هر نقطه قوت بایستی بین 5 تا 50 کاراکتر باشد"
        });
        new window.Tags(".js-textarea-tags-cons", "after", {
            min: 5,
            max: 50,
            message:
                "متن وارد شده در هر نقطه ضعف بایستی بین ۵ تا ۵۰ کاراکتر باشد"
        });
    ...
    initTooltips: function() {
        ...
        const tooltipInputContainers = document.querySelectorAll(
            ".js-tooltip-input"
        );
    ...
    extendValidator: function() {
        ...
        $.validator.addMethod(
            "persian_letters_only",
            function(value) {
                if ($.trim(value).length === 0) {
                    return true;
                }

                return /[ابپتثجچحخدذرزژسشصضطظعغفقکكگلمنوهیئي]$/i.test(value);
            },
        ...
        $.validator.addMethod(
            ...
            function(value) {
                ...
                return /[abcdefghijklmnopqrstuvwxyz]$/i.test(value);
            },
    ...
    showTooltip: function(e) {
        ...
        if (
            targetPosition.left + tooltipHalfMaxWidth + targetHalfWidth >=
            $body.innerWidth()
        ) {
            $tooltip.addClass("c-content-tooltip--left");
        } else if (
            targetPosition.left + targetHalfWidth - tooltipHalfMaxWidth <=
            0
        ) {
        ...
        if (
            ...
            $tooltip.css(
                "top",
                targetPosition.top - $tooltip.outerHeight() + "px"
            );
        } else {
            $tooltip.css(
                "top",
        ...
        if ($target.hasClass("js-tooltip-input")) {
            ...
            $tooltip.css(
                "left",
                targetPosition.left + $target.outerWidth() - 20 + "px"
            );
        } else {
            $tooltip.css(
                "left",
                targetPosition.left + $target.outerWidth() / 2 + "px"
            );
        }
    ...
    initCategoryThreeView: function() {
        ...
        $("#categoriesContainer").on("click", ".js-category-link", function(e) {
            ...
            $("#stepCategoryContainer")
                .find(".js-continue-btn")
                .addClass("disabled");
    ...
    initCategorySearchView: function() {
        ...
        $("#categoriesContainer").on(
            "change",
            ".js-category-change",
            function() {
                let $this = $(this);
                let leafCategoryId = $this.val();
                let $selectedCategoriesContainers = $this
                    .parent()
                    .find("li.c-content-categories__selected-category");

                let $selectedCategoriesPathContainer = $(
    ...
    selectCategoryFromThree: function($category) {
        ...
        let $selectedCategoriesContainers = $(
    ...
    initCategoryNextStep: function() {
        ...
        $("#categoryStepNext").on("click", function() {
            ...
            let $selectedCategoryIdConfirmed = $(
                "#selectedCategoryIdConfirmed"
            );
            let selectedCategoryIdConfirmedValue = $selectedCategoryIdConfirmed.val();
            let selectedCategoryIdConfirmedValueChangeApproved = $selectedCategoryIdConfirmed.data(
                "change-approved"
            );
            ...
            if (selectedCategoryIdConfirmedValue > 0) {
                if ($selectedCategoryId !== selectedCategoryIdConfirmedValue) {
                    if (!selectedCategoryIdConfirmedValueChangeApproved) {
                        const confirmModal = window.UIkit.modal(
                            "#confirmCategoryChangeModal"
                        );
                        new Promise(function(resolve, reject) {
                            let $selectedCategoryThemeConfirmed = $(
                                "#selectedCategoryThemeConfirmed"
                            ).val();

                            if (
                                $selectedCategoryThemeConfirmed &&
                                $("#selectedCategoryTheme").val() !==
                                    $selectedCategoryThemeConfirmed
                            ) {
                                $(
                                    "#differentCategoryThemeMessageContainer"
                                ).removeClass("hidden");
                            } else {
                                $(
                                    "#differentCategoryThemeMessageContainer"
                            ...
                            $(".js-accept").on("click", function() {
                                ...
                                if (isModuleActive("ccp_guideline")) {
                                    $(".js-guideline-icon").each(function(
                                        index,
                                        item
                                    ) {
                                        $(item).hasClass("uk-hidden")
                                            ? ""
                            ...
                            .then(function() {
                                $selectedCategoryIdConfirmed.data(
                                    "change-approved",
                                    true
                                );
                                $that.categoryNextStep($selectedCategoryId);
                                $("#stepAttributesForm").html("");
                                let $stepAttributesAccordion = $(
                                    "#stepAttributesAccordion"
                                );
    ...
    initDimension: function(dimensionLevel, dimensionConfig) {
        if (dimensionLevel) {
            ...
            $('[name="categoryTitleSpan"]').each(function() {
                $(this).html(
                    $(".c-content-categories__selected-category:last").html()
                );
            });
        }
    },
    ...
    categoryNextStep: function($selectedCategoryId) {
        ...
        Services.ajaxPOSTRequestJSON(
            ...
            function(data) {
                ...
                $that.initDimension(
                    data.dimension_level,
                    data.dimension_config
                );
                ...
                if (data.fields !== "undefined") {
                    if (data.fields.select !== "undefined") {
                        $.each(data.fields.select, function(selectSelector) {
                            ...
                            $.each(this.options, function(
                                ...
                                if (
                                    typeof optionData.selected !== "undefined"
                                ) {
                                    $option.prop(
                                        "selected",
                                        optionData.selected
                    ...
                    if (data.fields.input !== "undefined") {
                        $.each(data.fields.input, function(
                            inputSelector,
                            inputValue
                        ) {
                    ...
                    if (data.fields.span !== "undefined") {
                        $.each(data.fields.span, function(
                            spanSelector,
                            spanHtml
                        ) {
                    ...
                    if (data.fields.checkboxes !== "undefined") {
                        $.each(data.fields.checkboxes, function(
                            checkboxSelector,
                            isChecked
                        ) {
                            let $checkbox = $(
                    ...
                    if (data.fields.extra.allow_fake !== "undefined") {
                        ...
                        if (data.fields.extra.allow_fake) {
                            $("#productIsFakeContainerLabel").removeClass(
                                "disabled"
                            );
                            ...
                            $("#productIsFakeContainerLabel").addClass(
                                "disabled"
                            );
                    ...
                    if (data.fields.select.sites !== "undefined") {
                        ...
                        if ($select.length) {
                            ...
                            if (data.fields.select.sites.options.length === 0) {
                                ...
                                $.each(
                                    data.fields.select.sites.options,
                                    function(optionIndex, optionData) {
                                        let $option = $("<option/>");
                                        $option.attr("value", optionData.value);
                                        $option.text(optionData.text);
                                        if (
                                            typeof optionData.selected !==
                                            "undefined"
                                        ) {
                                            $option.prop(
                                                "selected",
                ...
                $that.scrollTo($stepContainer, 55);
                $("#stepCategoryAccordion .c-content-progress")
    ...
    initAttributesFormValidator: function() {
        ...
        $(".js-attribute-old-value").each(function() {
            $(this).rules("add", {
                ...
                messages: {
                    not_same_as_old_value:
                        "content.creation.forms.attributes.attributes_id.validation.same_are_moderated"
                }
            });
        });
    ...
    initTitleFormValidation: function() {
        ...
        let messages = {
            "title[title_fa]": {
                ...
                minlength:
                    "عنوان فارسی وارد شده کوتاه‌تر از حد مجاز است، این عنوان نمی‌تواند کوتاه‌تر از 7 کاراکتر باشد",
                maxlength:
                    "عنوان فارسی وارد شده طولانی‌تر از حد مجاز است، این عنوان نمی‌تواند طولانی‌تر از 255 کاراکتر باشد"
            },
            "title[title_en]": {
                validate_only_english:
                    "از کاراکترهای انگلیسی برای عنوان انگلیسی استفاده کنید",
                minlength:
                    "عنوان انگلیسی وارد شده کوتاه‌تر از حد مجاز است، این عنوان نمی‌تواند کوتاه‌تر از 7 کاراکتر باشد",
                maxlength:
                    "عنوان انگلیسی وارد شده طولانی‌تر از حد مجاز است، این عنوان نمی‌تواند طولاتی‌تر از 255 کاراکتر باشد"
            }
        };
    ...
    initProductChanges: function() {
        ...
        $("#brandsSelect").change(function() {
            ...
            if (
                $productIsFake.length &&
                $productIsFake.prop("checked") &&
                $productIsFake.data("brand-other-id").toString() !== value
            ) {
    ...
    initProductFormValidation: function() {
        ...
        if (
            ...
            $(".js-step-product-title").css("display") !== "none"
        ) {
        ...
        if (this.dimensionConfig) {
            dimensionValidationMessages.width.min =
                "ابعاد وارد شده در بازه صحیح ابعاد این گروه کالایی قرار ندارد.";
            dimensionValidationMessages.width.max =
                "ابعاد وارد شده در بازه صحیح ابعاد این گروه کالایی قرار ندارد.";
            dimensionValidationMessages.height.min =
                "ابعاد وارد شده در بازه صحیح ابعاد این گروه کالایی قرار ندارد.";
            dimensionValidationMessages.height.max =
                "ابعاد وارد شده در بازه صحیح ابعاد این گروه کالایی قرار ندارد.";
            dimensionValidationMessages.length.min =
                "ابعاد وارد شده در بازه صحیح ابعاد این گروه کالایی قرار ندارد.";
            dimensionValidationMessages.length.max =
                "ابعاد وارد شده در بازه صحیح ابعاد این گروه کالایی قرار ندارد.";

            dimensionValidationMessages.weight.min =
                "وزن وارد شده در بازه صحیح وزن این گروه کالایی قرار ندارد.";
            dimensionValidationMessages.weight.max =
                "وزن وارد شده در بازه صحیح وزن این گروه کالایی قرار ندارد.";
        }
        ...
        let messages = {
            ...
            "product[model]": {
                required: "وارد کردن مدل کالا اجباری است",
                minlength:
                    "مدل وارد شده کوتاه‌تر از حد مجاز است، مدل نمی‌تواند کوتاه‌تر از 2 کاراکتر باشد",
                maxlength:
            ...
            "product[title_alt]": {
                maxlength:
                    "عناوین دیگر طولانی‌تر از حد مجاز است، عناوین دیگر نمی‌تواند طولانی‌تر از 255 کاراکترباشد"
            },
            "product[package_weight]": {
                required: "وارد کردن وزن بسته‌بندی اجباری است",
                digits:
                    "فقط مجاز به وارد کردن عدد  و استفاده از اعداد صحیح برای وزن هستید",
            ...
            "product[package_length]": {
                ...
                digits:
                    "فقط مجاز به وارد کردن عدد  و استفاده از اعداد صحیح برای طول هستید",
            ...
            "product[package_width]": {
                ...
                digits:
                    "فقط مجاز به وارد کردن عدد  و استفاده از اعداد صحیح برای عرض هستید",
            ...
            "product[package_height]": {
                ...
                digits:
                    "فقط مجاز به وارد کردن عدد  و استفاده از اعداد صحیح برای ارتفاع هستید",
                min: dimensionValidationMessages.height.min,
                max: dimensionValidationMessages.height.max
            },
            "product[description]": {
                minlength:
                    "متن شرح کوتاه‌تر از حد مجاز است، شرح کالا نمی‌تواند کوتاه‌تر از 150 کاراکتر باشد",
                maxlength:
                    "متن شرح طولانی‌تر از حد مجاز است، شرح کالا نمی‌تواند طولانی‌تر از 2000 کاراکتر باشد"
            }
        };
    ...
    initProductNextStep: function() {
        ...
        $("#productIsFake").on("click", function() {
            ...
            if ($this.prop("checked")) {
                ...
                const confirmModal = window.UIkit.modal(
                    "#confirmFakeSelectionBrandChangeModal"
                );
                new Promise(function(resolve, reject) {
                    confirmModal.show();
                    $(".js-accept").on("click", function() {
    ...
    initTitleNextStep: function() {
        ...
        $editBtn.on("click", function() {
            $(".js-edite-title-suggested")
                .removeClass("uk-hidden disabled")
                .addClass("uk-open");
        ...
        Services.ajaxPOSTRequestJSON(
            ...
            function(data) {
                if (data.attributesForm.bind.autoTitleSuggestion) {
                    ...
                    $(".js-edit-mode-suggested-title-fa").val(
                        data.attributesForm.bind.autoTitleSuggestion.fa
                    );
                    $(".js-edit-mode-suggested-title-en").val(
                        data.attributesForm.bind.autoTitleSuggestion.en
                    );
                    if ($autoTitleField.hasClass("disabled")) {
                        $autoTitleField
                            .find(".js-suggested-title-fa")
                            .val(
                                data.attributesForm.bind.autoTitleSuggestion.fa
                            );
                        $autoTitleField
                            .find(".js-suggested-title-en")
                            .val(
                                data.attributesForm.bind.autoTitleSuggestion.en
                            );
                        if (
                            $autoTitleField.data("status") === "draft" ||
                            $autoTitleField.data("status") === "in_review"
                        ) {
                            $autoTitleField.addClass("uk-hidden");
                        }
                    }

                    $("#cancelEditSubjectSuggested")
                        .removeClass("uk-hidden disabled")
                        .attr("data-edit-mode", true);
                    $("#setSubjectStepNext")
                        .html("تأیید عنوان و ذحیره")
    ...
    initSave: function() {
        ...
        $("#saveButton").on("click", function() {
            ...
            if (
                ...
                isModuleActive("auto_title_suggestion") &&
                $(".js-edit-mode-title-fa").val() === "" &&
                !$(".js-edite-title-suggested").data("edit-mode")
            ) {
                $(".js-edit-mode-title-fa").val(
                    $(".js-edit-mode-suggested-title-fa").val()
                );
                $(".js-edit-mode-title-en").val(
                    $(".js-edit-mode-suggested-title-en").val()
                );
            }

            if (
                $(".js-product-model").length &&
                $(".js-product-model").hasClass("uk-hidden")
            ) {
                if (
                    $(".js-model-field").val() === "-" ||
                    $(".js-model-field").val() === "/"
            ...
            let $afterModerationErrorImageContainer = $(
                "#afterModerationErrorImageContainer"
            );
            ...
            if (
                $(".js-edite-title-suggested").length &&
                $(".js-edite-title-suggested").data("edit-mode")
            ) {
                if (
                    !$(".js-edite-title-suggested")
                        .parents("section#stepTitleAccordion")
                        .hasClass("disabled")
                ) {
            ...
            Services.ajaxPOSTRequestJSON(
                ...
                function(data) {
                    ...
                    if (
                        typeof data.save !== "undefined" &&
                        typeof data.save.status !== "undefined"
                    ) {
            ...
            Services.ajaxPOSTRequestJSON(
                ...
                function(data) {
                    ...
                    $.each(data, function(videoPosition, videoData) {
                        let $suggestionItem = $(
                            "#suggestionTemplateVideos > li"
                        ).clone();

                        $suggestionItem.data("id", videoData.id);
                        $suggestionItem
                            .find(".js-suggestion-img")
                            .attr("src", videoData.cover);
                        $suggestionItem
                            .find(".js-suggestion-name")
                            .text(videoData.title);
                        $suggestionItem
                            .find(".js-suggestion-id")
                            .text(videoData.id);
                        $suggestionItem
    ...
    imagesRequired: function() {
        if (this.isContentUser()) {
            ...
            if (this.isNewMode() || this.isEditBeforeSubmitMode()) {
                ...
                return (
                    $sellerWillUpload.length === 0 ||
                    $sellerWillUpload.prop("checked") === true
                );
            }
    ...
    handleErrors: function(data) {
        ...
        if (
            ...
            if (typeof data.categoryForm.errors !== "undefined") {
                if (typeof data.categoryForm.errors !== "undefined") {
                    $.each(data.categoryForm.errors, function(
                        messageKey,
                        messageText
                    ) {
                        let $p = $("<p/>");
        ...
        if (
            ...
            if (typeof data.productForm.errors !== "undefined") {
                ...
                $.each(data.productForm.errors, function(
                    messageKey,
                    messageText
                ) {
                    let $p = $("<p/>");
        ...
        if (
            ...
            this.attributeFormValidator.showErrors(
                data.attributesForm.jsErrors
            );
            if ($scrollTo == null) {
        ...
        if (
            typeof data.imagesFormValid !== "undefined" &&
            data.imagesFormValid !== true
        ) {
            ...
            if (typeof data.imagesForm.globalErrors !== "undefined") {
                ...
                if (
                    typeof data.imagesForm.globalErrors.global_main_image !==
                    "undefined"
                ) {
                    $mainImageErrorContainer.removeClass("hidden");
                    if ($scrollTo == null) {
                ...
                if (!$.isEmptyObject(data.imagesForm.globalErrors)) {
                    ...
                    $.each(data.imagesForm.globalErrors, function(
                        messageKey,
                        messageText
                    ) {
            ...
            $imagesContainer.find("li").each(function() {
                $(this)
                    .find("ul.js-upload-error-list:first")
                    .html("");
                $(this)
                    .find("ul.js-upload-error-list:first")
                    .css("display", "none");
            ...
            if (typeof data.imagesForm.errors !== "undefined") {
                ...
                $.each(data.imagesForm.errors, function(elementId, errorText) {
                    let $errorsContainer = $("#" + elementId).find(
                        "ul.js-upload-error-list:first"
                    );
                    let $li = $("<li/>");
                    $li.text(errorText);
    ...
    initImagesUpload: function() {
        ...
        window.UIkit.upload($uploadGalleryContainer, {
            ...
            beforeAll: function() {
                ...
                for (
                    let i = $allFiles.length - $file.length;
                    i < $allFiles.length;
                    i++
                ) {
                    ...
                    $uploadsItem
                        .find(".js-upload-name")
                        .text($allFiles[i].name);
                    $uploadsItem
                        .find(".js-upload-size")
                        .text(Math.round($allFiles[i].size / 1024) + " KB");
                }
                $that.initUiSelect();
            ...
            complete: function() {
                ...
                let $uploadFileSection = $(
                    "#uploadedFileId_" + result.data.slot
                );

                let $errorsSection = $uploadFileSection.find(
                    "ul.js-upload-error-list:first"
                );
                $errorsSection.html("");
                ...
                if (typeof result.data.errors !== "undefined") {
                    ...
                    $.each(result.data.errors, function(
                        messageKey,
                        messageText
                    ) {
                        let $li = $("<li/>");
                ...
                let $loader = $uploadFileSection.find(
                ...
                $imageInput.attr(
                    "name",
                    "images[" + $idKeyPrefix + "images][]"
                );
                $imageInput.val(result.data.id);
                $uploadFileSection.prop("id", $idKeyPrefix + result.data.id);

                if ($that.isContentUser()) {
                    let $imageTypeSelect = $uploadFileSection.find(
                        ".js-image-type-select:first"
                    );
                    $imageTypeSelect.attr(
                        "name",
                        "images[type_" + $idKeyPrefix + result.data.id + "]"
                    );

                    let $imageActiveCheckbox = $uploadFileSection.find(
                        ".js-image-active-checkbox:first"
                    );
                    $imageActiveCheckbox.attr(
                        "name",
                        "images[active_" + $idKeyPrefix + result.data.id + "]"
                    );

                    let $imageWatermarkCheckbox = $uploadFileSection.find(
                        ".js-image-watermark-checkbox:first"
                    );
                    $imageWatermarkCheckbox.attr(
                        "name",
                        "images[watermark_" +
                            $idKeyPrefix +
                            result.data.id +
                    ...
                    let $imageCopyrightCheckbox = $uploadFileSection.find(
                        ".js-image-copyright-checkbox:first"
                    );
                    $imageCopyrightCheckbox.attr(
                        "name",
                        "images[copyright_" +
                            $idKeyPrefix +
                            result.data.id +
    ...
    initImages3dUpload: function() {
        ...
        window.UIkit.upload($this, {
            ...
            complete: function() {
                ...
                let hasError = false;
                let $errorsSection = $uploadsItem.find(
                    "ul.js-upload-error-list:first"
                );
                $errorsSection.html("");
                $errorsSection.removeClass("has-error");
                ...
                if (
                    !hasError &&
                    typeof result.data.error_server !== "undefined"
                ) {
                    hasError = true;
                    $uploadsItem.addClass("has-error");
                    let $li = $("<li/>");
    ...
    initSortableFiles: function() {
        $(".js-sortable-list").each(function() {
            ...
            $this.on("moved", function() {
                ...
                let $rows = $this.find(
                    ".c-content-upload__gallery-row:not(.primary):not(.afterimage):not(.loading):not(.has-error)"
                );
                $rows
    ...
    initUploadsControlsHandler: function() {
        ...
        $(document).on("click", ".js-remove-upload", function() {
            ...
            if ($row.attr("id") === "upload3dItem") {
                ...
                if (
                    isModuleActive("not_production") &&
                    $("#videoContainer").find(".js-video-id-input").length < 2
                ) {
                    $row.find(".js-video-id-input:first").attr(
                        "name",
                        "videos[videos]"
                    );
                    $row.find(".js-video-id-input:first").val("");
                } else {
                    $row.find(".js-video-id-input:first").prop(
                        "disabled",
                        true
                    );
                }
        ...
        $(document).on("click", ".js-cancel-upload", function() {
            ...
            if (is3dImage) {
                ...
                if (
                    typeof $that.data.galleryRequests[slot - 1] === "undefined"
                ) {
                    return;
                }
        ...
        $(document).on("click", ".js-flag-primary", function() {
            ...
            $imagesContainer
                .find(".js-flag-primary:first")
                .removeClass("checked");
            $imagesContainer
                .find(".js-image-options:first")
                .removeClass("hidden");
        ...
        $(document).on("click", ".js-undo-remove", function() {
            ...
            if ($row.attr("id") === "upload3dItem") {
                ...
                if (isModuleActive("not_production")) {
                    $row.find(".js-video-id-input:first").val(
                        $row.find(".js-video-id-input:first").data("id")
                    );
                } else {
                    $row.find(".js-video-id-input:first").prop(
                        "disabled",
    ...
    initSelectVideosHandler: function() {
        ...
        $(document).on("click", ".js-video-suggestion", function() {
            ...
            $suggestionItem
                .find(".js-video-img")
                .attr("src", $this.find(".js-suggestion-img").attr("src"));
            $suggestionItem
                .find(".js-video-name")
                .text($this.find(".js-suggestion-name").text());
            $suggestionItem.find(".js-video-id").text(id);
            $suggestionItem
                .find(".js-video-duration")
                .text($this.find(".js-video-duration").text());
        ...
        if (!trigger) {
            ...
            $containers.forEach(function(e) {
                let $rows = $(e)
                    ...
                    .find(
                        ".c-content-upload__gallery-row:not(.primary):not(.afterimage)"
                    );
                $rows
                    .removeClass("first last")
                    .first()
    ...
    initNewBrandRequestModal: function() {
        ...
        let $formValidator = $form.validate({
            rules: {
                ...
                "brand[registration_image_id]": {
                    required: function() {
                        ...
                        return (
                            $selectedOrigin.val() === "iranian" &&
                            $("#registrationUrlValue").val().length < 5
                        );
                    }
                },
                "brand[registration_url]": {
                    required: function() {
                        ...
                        return (
                            $selectedOrigin.val() === "iranian" &&
                            $("#registrationImageTempId").val().length === 0
                        );
                    },
                    minlength: 5,
                    maxlength: 255
            ...
            messages: {
                "brand[name_fa]": {
                    ...
                    minlength:
                        "عنوان فارسی برند نمی‌تواند کوتاه‌تر از 2 کاراکتر باشد",
                    maxlength:
                        "عنوان فارسی برند نمی‌تواند طولانی‌تر از 255 کاراکتر باشد",
                    persian_letters_only: "تنها حروف فارسی مجاز است"
                },
                "brand[name_en]": {
                    required: "وارد کردن عنوان انگلیسی برند اجباری است",
                    minlength:
                        "عنوان انگلیسی برند نمی‌تواند کوتاه‌تر از 2 کاراکتر باشد",
                    maxlength:
                        "عنوان فارسی برند نمی‌تواند طولانی‌تر از 255 کاراکتر باشد",
                ...
                "brand[description]": {
                    ...
                    minlength:
                        "متن وارد شده برای شرح برند نمی‌تواند کوتاه‌تر از 100 کاراکتر باشد",
                    maxlength:
                        "متن وارد شده برای شرح برند نمی‌تواند طولانی‌تر از 750 کاراکتر باشد"
                },
                "brand[site]": {
                ...
                "brand[url]": {
                    ...
                    minlength:
                        "آدرس وارد شده نمی‌تواند کوتاه‌تر از 5 کاراکتر باشد",
                    maxlength:
                        "آدرس وارد شده نمی‌تواند طولانی‌تر از 255 کاراکتر باشد"
                },
                "brand[logo_id]": {
        ...
        $("#saveBrandRequestButton").click(function() {
            ...
            $("#newBrandRequestCategoryIdContainer").val(
                $("#selectedCategoryIdConfirmed").val()
            );
            ...
            Services.ajaxPOSTRequestJSON(
                ...
                function(data) {
                    ...
                    if (typeof data.globalErrors !== "undefined") {
                        $.each(data.globalErrors, function(
                            messageKey,
                            messageText
                        ) {
                            let $div = $("<div/>");
    ...
    initGuidelineModalAction: function() {
        ...
        $(".js-show-video").on("click", function() {
            $(this)
                .parents("#guidelineModalContent")
                .find('.js-modal-item[data-item="items"]')
                .fadeOut("slow", "swing", function() {
                    $(this).addClass("uk-hidden");
                });
            $(this)
                .parents("#guidelineModalContent")
                .find('.js-modal-item[data-item="video"]')
    ...
    initGuidelineModals: function(data) {
        ...
        Object.entries(data).forEach(function([key, value]) {
            ...
            $(`.js-guideline-icon[data-guideline-modal=${key}]`).removeClass(
                "uk-hidden"
            );
                ...
                if (sectionValue) {
                    switch (sectionKey) {
                        case "short_description":
                            $sectionModal
                                .find(`.js-modal-item[data-item=${sectionKey}]`)
                                .empty()
                                .html(sectionValue);
                            $sectionModal
                                .find(`.js-modal-item[data-item=${sectionKey}]`)
                        ...
                        case "video":
                            $sectionModal
                                .find(
                                    `.js-modal-item[data-item=${sectionKey}] > figure > video > source`
                                )
                                .attr("src", sectionValue);
                            if (value.items || value.gallery) {
                                ...
                                $sectionModal
                                    .find(
                                        `.js-modal-item[data-item=${sectionKey}]`
                                    )
                                    .removeClass("uk-hidden");
                            }
                            break;
        ...
        function generateItems($items, $modal) {
            var $item = $modal
                .find('.js-modal-item[data-item="items"] > div')
                .first()
                .clone();
            var $modalItemsContainer = $modal.find(
                '.js-modal-item[data-item="items"]'
            );
            $modalItemsContainer.empty();
            for (var val of $items) {
                $item
                    .find(".js-expand-item > p")
                    .empty()
                    .html(val.title);
                $item
                    .find(".js-guideline-desc")
                    .empty()
                    .append(val.content);
