...
final class postPishtaz {
    ...
    public function getPrice( $codRequest = false ){
        $bime = config("postPishtaz.bime") ;
        $tax  = config("postPishtaz.maliat") ;

        $inSideKarmozd = config("postPishtaz.inSideKarmozd") ;
        $outSideKarmozd = config("postPishtaz.outSideKarmozd") ;
        $haghSabt = config("postPishtaz.haghsabt") ;
        $getPart = $this->setPart() ;
        $cod = config("postPishtaz.cod") ;
        $cost = $this->cost ;
        $price = $this->setPrice() ;
    ...
    private function setPrice(){
        ...
        $price = config(sprintf("postPishtaz.%s.tariff.%s.%s" , $type , $weightType , $partType )) ;

        if(  in_array( $weightType , ["higher" , 5000] ) ){
            $tariff = config( sprintf("postPishtaz.%s.tariff" , $type ) ) ; // از تنظیمات تعرفه رو گرفتیم
            ...
            if( !($weightType == 5000  && $partType == "insidePart" ) ){
